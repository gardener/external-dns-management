...
var _ = Describe("Compound controller tests", func() {
	...
	var (
		...
		checkSingleEntryInMockDatabase = func(entry *v1alpha1.DNSEntry) {
			...
			for _, zoneDump := range dump.InMemory {
				...
				case zoneDump.HostedZone.Domain == "first.example.com" && entry == nil:
					Expect(zoneDump.DNSSets).To(BeEmpty(), "unexpected number of DNS sets in first.example.com")
				case zoneDump.HostedZone.Domain == "first.example.com" && entry != nil:
					Expect(zoneDump.DNSSets).To(HaveKey(dns.DNSSetName{DNSName: entry.Spec.DNSName}))
					Expect(zoneDump.DNSSets).To(HaveLen(1), "unexpected number of DNS sets in first.example.com")
					set := zoneDump.DNSSets[dns.DNSSetName{DNSName: entry.Spec.DNSName}]
					Expect(set.Sets).To(HaveKey("A"))
					Expect(set.Sets).To(HaveLen(1))
					setA := set.Sets["A"]
					Expect(setA.Records).To(ConsistOf(
						PointTo(MatchFields(IgnoreExtras, Fields{
							"Value": Equal(entry.Spec.Targets[0]),
						}))))
					Expect(setA.Type).To(Equal("A"))
					Expect(setA.TTL).To(Equal(int64(defaultTTL)))
				case zoneDump.HostedZone.Domain == "second.example.com":
					Expect(zoneDump.DNSSets).To(BeEmpty(), "unexpected number of DNS sets in second.example.com")
				default:
		...
		checkEntry = func(entry *v1alpha1.DNSEntry) {
			Eventually(func(g Gomega) {
				...
				g.Expect(entry.Status.Text).To(Equal(quoted(entry.Spec.Text)))
	...
	It("should deal with temporary backend failures on deleting an entry", func() {
		...
		deleteSet.Sets.AddRecord("AAAA", "1::2", 42)
		failID := mock.TestMock[testRunID].AddApplyFailSimulation(firstZoneID, &provider.ChangeRequest{
