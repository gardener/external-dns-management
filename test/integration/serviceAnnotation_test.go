...
var _ = Describe("ServiceAnnotation", func() {
	It("creates DNS entry", func() {
		...
		svc4, err := testEnv.CreateServiceWithAnnotation("mysvc4", svcDomain4, status, ttl, &routingPolicy, nil)
		Ω(err).ShouldNot(HaveOccurred())
		...
		testEnv.AnnotateObject(svc, "dns.gardener.cloud/ignore", "reconcile")
		testEnv.AwaitEntryState(entryObj.GetName(), "Ignored")
		testEnv.AnnotateObject(svc, "dns.gardener.cloud/ignore", "")
		testEnv.AwaitEntryState(entryObj.GetName(), "Ready")
		testEnv.AnnotateObject(svc, "dns.gardener.cloud/ignore", "full")
		testEnv.AwaitEntryState(entryObj.GetName(), "Ignored")
		testEnv.AnnotateObject(svc, "dns.gardener.cloud/ignore", "")
		testEnv.AwaitEntryState(entryObj.GetName(), "Ready")
