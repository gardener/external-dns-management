...
func NewHandler(c *provider.DNSHandlerConfig) (provider.DNSHandler, error) {
	...
	h.zonesClient, err = armprivatedns.NewPrivateZonesClient(subscriptionID, tc, opts)
	if err != nil {
		return nil, err
	}
	h.recordsClient, err = armprivatedns.NewRecordSetsClient(subscriptionID, tc, opts)
	if err != nil {
		return nil, err
	}
	...
	_, err = h.zonesClient.NewListPager(&armprivatedns.PrivateZonesClientListOptions{Top: ptr.To[int32](1)}).NextPage(h.ctx)
	if err != nil {
