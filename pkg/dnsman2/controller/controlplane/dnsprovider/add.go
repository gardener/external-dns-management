...
func (r *Reconciler) AddToManager(mgr manager.Manager, controlPlaneCluster cluster.Cluster, cfg *config.DNSManagerConfiguration) error {
	...
	log.Info("Supported provider types", "providerTypes", strings.Join(r.DNSHandlerFactory.GetSupportedTypes(), ","))
		...
		bld.WatchesRawSource(
			source.Channel(ch,
				handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, _ client.Object) []reconcile.Request {
					return r.allProvidersToReconcile(ctx)
				}),
			),
		)
...
func (r *Reconciler) allProvidersToReconcile(ctx context.Context) []reconcile.Request {
