...
var _ = Describe("Reconciler", func() {
	...
	var (
		...
		testWithoutCreation = func(target *dnsv1alpha1.DNSProviderSpec, offset int, expectedErrorMessage ...string) *dnsv1alpha1.DNSProvider {
			...
			actualSourceProvider := items[0]
			ExpectWithOffset(offset+1, actualSourceProvider).NotTo(BeNil(), "DNS provider not found")
			ExpectWithOffset(offset+1, actualSourceProvider.Namespace).To(Equal(*reconciler.Config.Controllers.Source.TargetNamespace))
			ExpectWithOffset(offset+1, actualSourceProvider.Name).To(ContainSubstring("foo-"))
			...
			case sameClusterID && *reconciler.Config.Controllers.Source.TargetNamespace != sourceProvider.Namespace:
				ExpectWithOffset(offset+1, actualSourceProvider.OwnerReferences).To(BeEmpty())
				ExpectWithOffset(offset+1, actualSourceProvider.Annotations["resources.gardener.cloud/owners"]).To(Equal(fmt.Sprintf("dns.gardener.cloud/Provider/%s/%s", sourceProvider.Namespace, sourceProvider.Name)))
			default:
				ExpectWithOffset(offset+1, actualSourceProvider.OwnerReferences).To(BeEmpty())
				ExpectWithOffset(offset+1, actualSourceProvider.Annotations["resources.gardener.cloud/owners"]).To(Equal(fmt.Sprintf("%s:dns.gardener.cloud/Provider/%s/%s", ptr.Deref(reconciler.Config.Controllers.Source.SourceClusterID, ""), sourceProvider.Namespace, sourceProvider.Name)))
			}

			ExpectWithOffset(offset+1, actualSourceProvider.Spec).To(Equal(*target))
			return actualSourceProvider
		}
	...
	BeforeEach(func() {
		...
		reconciler.GVK = corev1.SchemeGroupVersion.WithKind(dnsv1alpha1.DNSProviderKind)
	...
	Describe("#Reconcile", func() {
		It("should create target DNSProvider object in same cluster", func() {
			test(&dnsv1alpha1.DNSProviderSpec{
				Type: mock.ProviderType,
				SecretRef: &corev1.SecretReference{
					Name: "foo-secret",
				},
