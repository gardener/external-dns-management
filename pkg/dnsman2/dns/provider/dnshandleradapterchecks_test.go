...
var _ = Describe("DNSHandlerAdapterChecks", func() {
	...
	Context("ValidateProperties", func() {
		...
		Context("SetDisjunctPropertySets - 3 way", func() {
			BeforeEach(func() {
			...
			It("should succeed if properties a and b are set", func() {
				props["foo"] = "good"
				props["a"] = "good"
				props["b-alias"] = "good"
				err := checks.ValidateProperties("test", props)
			...
			It("should succeed if property c is set", func() {
				props["foo"] = "good"
				props["c"] = "good"
				err := checks.ValidateProperties("test", props)
				Expect(err).To(Succeed())
			...
			It("should succeed if property d is set", func() {
				props["foo"] = "good"
				props["d"] = "good"
				err := checks.ValidateProperties("test", props)
				Expect(err).To(Succeed())
			...
			It("should not succeed if property c and d are set", func() {
				props["foo"] = "good"
				props["c"] = "good"
				props["d"] = "good"
				err := checks.ValidateProperties("test", props)
			...
			It("should not succeed if property a and c are set", func() {
				props["foo"] = "good"
				props["a-alias"] = "good"
				props["c"] = "good"
				err := checks.ValidateProperties("test", props)
