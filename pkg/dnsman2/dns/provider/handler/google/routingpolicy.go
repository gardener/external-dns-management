...
type routingPolicyChanges map[dnsname]map[dns.RecordType]*googledns.ResourceRecordSet
...
func (c routingPolicyChanges) addChange(set *googledns.ResourceRecordSet, add bool) {
	perType := c[set.Name]
	if perType == nil {
		perType = map[dns.RecordType]*googledns.ResourceRecordSet{}
	}
	current := perType[toRecordType(set.Type)]
	...
	perType[toRecordType(set.Type)] = current
	c[set.Name] = perType
}

func routingPolicyFromRRS(set *googledns.ResourceRecordSet) dns.RoutingPolicyType {
...
func (c routingPolicyChanges) calcDeletionsAndAdditionsWrr(
	rrsetGetter rrsetGetterFunc, name dnsname, typ dns.RecordType,
	...
	for i := len(rrset.RoutingPolicy.Wrr.Items) - 1; i >= 0; i-- {
		if rrset.RoutingPolicy.Wrr.Items[i] == _deleteddeletedMarkerWrr_ {
			...
			rrset.RoutingPolicy.Wrr.Items[i] = createWrrPlaceHolderItem(typ)
		} else if maxIndex == i && isWrrPlaceHolderItem(typ, rrset.RoutingPolicy.Wrr.Items[i]) {
			rrset.RoutingPolicy.Wrr.Items = rrset.RoutingPolicy.Wrr.Items[:i]
			maxIndex = i - 1
		}
	}
...
func extractRoutingPolicy(dnsSetName dns.DNSSetName, rs *dns.RecordSet) (*googleRoutingPolicyData, error) {
	if dnsSetName.SetIdentifier == "" && rs.RoutingPolicy == nil {
		return nil, nil
	}
	if dnsSetName.SetIdentifier == "" {
	...
	case dns.RoutingPolicyWeighted:
		...
		index, err = strconv.Atoi(dnsSetName.SetIdentifier)
		if index < 0 || index >= routingPolicyMaxIndices || err != nil {
			return nil, fmt.Errorf("for %s with weighted routing policy, the setIdentifier must be an number >= 0 and < %d, but got: %s", ProviderType, routingPolicyMaxIndices, dnsSetName.SetIdentifier)
		}
	case dns.RoutingPolicyGeoLocation:
		keys = []string{keyLocation}
		if dnsSetName.SetIdentifier != rs.RoutingPolicy.Parameters[keyLocation] {
			return nil, fmt.Errorf("for %s with geolocation routing policy, the setIdentifier must be identical to the location, but: %s != %s", ProviderType, dnsSetName.SetIdentifier, rs.RoutingPolicy.Parameters[keyLocation])
		}
	default:
		return nil, fmt.Errorf("unsupported routing policy: %s", rs.RoutingPolicy.Type)
	}

	if err := rs.RoutingPolicy.CheckParameterKeys(keys, nil); err != nil {
		return nil, err
	}

	data := &googleRoutingPolicyData{index: index}
	for key, value := range rs.RoutingPolicy.Parameters {
		switch key {
		case keyWeight:
...
func describeRoutingPolicy(rrset *googledns.ResourceRecordSet) string {
	...
	if rrset.RoutingPolicy.Wrr != nil {
		...
		for i, item := range rrset.RoutingPolicy.Wrr.Items {
			if !isWrrPlaceHolderItem(toRecordType(rrset.Type), item) {
				_, _ = fmt.Fprintf(buf, "wrr:[%d]%.1f:%s;", i, item.Weight, strings.Join(item.Rrdatas, ","))
			}
		}
		return buf.String()
	}
	...
	if rrset.RoutingPolicy.Geo != nil {
		buf := new(bytes.Buffer)
		for _, item := range rrset.RoutingPolicy.Geo.Items {
			_, _ = fmt.Fprintf(buf, "geo:%s:%s;", item.Location, strings.Join(item.Rrdatas, ","))
		}
		return buf.String()
	}
...
func createWrrPlaceHolderItem(typ dns.RecordType) *googledns.RRSetRoutingPolicyWrrPolicyWrrPolicyItem {
...
func isWrrPlaceHolderItem(typ dns.RecordType, item *googledns.RRSetRoutingPolicyWrrPolicyWrrPolicyItem) bool {
	return item.Weight == 0 && len(item.Rrdatas) == 1 && item.Rrdatas[0] == rrDefaultValue(typ)
}

func rrDefaultValue(typ dns.RecordType) string {
	switch typ {
	case dns.TypeTXT:
		return "\"__dummy__\""
	case dns.TypeA:
...
func toRecordType(typ string) dns.RecordType {
	switch typ {
