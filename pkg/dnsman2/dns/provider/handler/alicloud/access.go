...
// GetDNSName returns the fully qualified domain name for a given record.
...
// GetRR returns the relative record name (RR) for a given fully qualified domain name and domain.
...
func (a *access) GetRecordList(_ context.Context, dnsName, rtype string, zone provider.DNSHostedZone) (raw.RecordList, []*dns.RoutingPolicy, error) {
	...
	consume := func(record *alidns.DescribeDomainRecordsResponseBodyDomainRecordsRecord) (bool, error) {
		if ptr.Deref(record.RR, "") == rr {
			rl = append(rl, (*Record)(record))
		}
		return true, nil
	}

	err := a.listRecords(zone.ZoneID().ID, zone.Domain(), consume, requestModifier)
	if err != nil {
		return nil, nil, err
	}
	...
	for i, r := range rl {
		if r.GetSetIdentifier() != "" {
			...
			default:
				return nil, nil, fmt.Errorf("unsupported routing policy type %q for record %q in zone %q", r.GetType(), r.GetDNSName(), zone.Domain())
			}
		}
	}
