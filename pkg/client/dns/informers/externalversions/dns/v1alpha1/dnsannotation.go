...
func NewDNSAnnotationInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredDNSAnnotationInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.DnsV1alpha1().DNSAnnotations(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.DnsV1alpha1().DNSAnnotations(namespace).Watch(context.TODO(), options)
			},
		},
...
func (f *dNSAnnotationInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredDNSAnnotationInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}
...
func (f *dNSAnnotationInformer) Lister() v1alpha1.DNSAnnotationLister {
	return v1alpha1.NewDNSAnnotationLister(f.Informer().GetIndexer())
}
